...
TEST(PermanentTest, RyserSparseSpaRyserCUDA) {
    int n = 10; 
    int nnz = static_cast<int>(n * n * 0.5);
    ...
    double naivePermanent = computePermanent(matrix);
    double ryserSpaRyser = computePermanentSpaRyserMain(n,nnz, crs_ptrs, crs_colids, crs_values, ccs_ptrs, ccs_rowids, ccs_values);
